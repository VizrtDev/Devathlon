...
/**
 * This class represents the first item of the witch, which
 * spawns a tornado and make some Damage
 *
 * @author Leon
 */
public class TornadoItem extends Item implements EventExecutor {
    public TornadoItem(  ) {
        super( Material.SLIME_BALL, "§cTornado", null );
...
//                ticker_horisontal = (float) ((ticker_horisontal + 0.8f) % 2*Math.PI);
    ...
    public void doEvent( Player player ) {
        spawnTornado( WitchHunter.getInstance(),
                PlayerUtil.staredAtBlock( player ).getLocation(),
